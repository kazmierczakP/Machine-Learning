...
def sequences_model():
    ...
    # Define early stopping callbacks (optional).
    # ADD CODE HERE
    early_stopping_1 = EarlyStopping(monitor='val_mae', mode='min', patience=10, verbose=1, min_delta=0.005)
    early_stopping_2 = MyCallback()

    # Train the model
    # ADD CODE HERE
    model.fit(x=train_set, epochs=200, verbose=1, validation_data=valid_set,
              callbacks=[early_stopping_1, early_stopping_2])
    # model.fit(x=train_set, epochs=200, verbose=1, validation_data=valid_set, callbacks=[early_stopping_2])
